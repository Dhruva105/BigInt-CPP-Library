...
// FIXED: Added 'const' to the argument type here
...
BigInt BigInt::operator++(int temp) {
    BigInt aux; aux = *this; ++(*this); return aux;
}
BigInt &BigInt::operator--() {
...
BigInt operator+(const BigInt &a, const BigInt &b) {
    BigInt temp; temp = a; temp += b; return temp;
}
...
BigInt operator^(BigInt & a, BigInt & b) {
    BigInt temp(a); temp ^= b; return temp;
}
...
BigInt sqrt(BigInt & a) {
    ...
    divide_by_2(right);
    while (left <= right) {
        mid += left; mid += right; divide_by_2(mid);
        prod = (mid * mid);
